...
void parallelWeightedAStar(const Environment *env, float depthPenalty, int numParallel, std::string socketName) {
	...
	open.push(new Node{env,0,-1,0,NULL}); //Push root node to open
	printf("GET START\n");
	closed.insert(new Node{env,0,-1,0,NULL}); //Add root node to seen
	...
	while (isSolved == false) {
		...
		for (int i=0; i<numPop; i++) {
			Node *node = open.top();
			isSolved = isSolved | node->env->isSolved();
			if (isSolved && (solvedNode == NULL)) {
				solvedNode = node;
			}
		...
		for (unsigned int i=0; i<popped.size(); i++) {
			...
			for (unsigned int j=0; j<children_env.size(); j++) {
				Node *node = new Node{children_env[j],depth,(int) j,0,popped[i]};
		...
		//std::thread writeThread (writeFile,inputFileName,children);
		//writeThread.join();
		...
		numNodesGenerated += nodesToAdd.size();
		...
		for (unsigned int i=0; i<nodesToAdd_idx.size(); i++) {
			values.push_back(values_temp[nodesToAdd_idx[i]]);
		}
		...
		printf("Times - remOpen: %f, exp: %f, write: %f, check: %f, heur: %f, cost: %f, add: %f\n",remOpenTime,expandingTime,dataWriteTime,checkClosedTime,heuristicTime,costTime,addToQueueTime);

		itrTime = getTimeElapsed(startTime,std::chrono::high_resolution_clock::now());

		printf("Iteration: %i, Min/Max - Depth: %i/%i, Value: %.2f/%.2f, Cost: %.2f/%.2f, OpenSize: %li, ClosedSize: %li, Time: %f\n\n",searchItr,minDepth,maxDepth,minValue,maxValue,minCost,maxCost,open.size(),closed.size(),itrTime);
		searchItr++;
	}
...
int main(int argc, const char *argv[]) {
    ...
    if (envName == "puzzle15") {
        env = new Puzzle15(init);
    } else if (envName == "puzzle24") {
        env = new Puzzle24(init);
    } else if (envName == "puzzle35") {
        env = new Puzzle35(init);
